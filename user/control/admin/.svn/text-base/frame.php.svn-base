...
/*
	...
	$Id: frame.php 12135 2008-01-14 03:47:01Z heyond $
*/
...
class control extends adminbase {
	...
	function onindex() {
		...
		$mainurl = !empty($mainurl) && preg_match("/^admin\.php\?(&*\w+=\w+)*$/i", $mainurl) ? $mainurl : 'admin.php?m=frame&a=main';
		$this->view->assign('mainurl', $mainurl);
		$this->view->display('admin_frame_index');
	}

	function onmain() {
		//header('location: '.UC_API.'/admin.php?m=setting&a=ls');
