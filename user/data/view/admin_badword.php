...
<div class="container">
	<? if($status) { ?>
		<div class="correctmsg"><p><? if($status == 2) { ?>词语过滤成功更新。<? } elseif($status == 1) { ?>词语过滤添加成功。<? } ?></p></div>
	<? } ?>
				...
				<table class="datalist fixwidth">
					...
					<? foreach((array)$badwordlist as $badword) {?>
						<tr>
							...
							<td class="tdinput"><input type="text" name="find[<?=$badword['id']?>]" value="<?=$badword['find']?>" title="点击编辑，提交后保存" class="txtnobd" onblur="this.className='txtnobd'" onfocus="this.className='txt'" /></td>
							<td class="tdarrow">&gt;</td>
							<td class="tdinput"><input type="text" name="replacement[<?=$badword['id']?>]" value="<?=$badword['replacement']?>" title="点击编辑，提交后保存" class="txtnobd"  onblur="this.className='txtnobd'" onfocus="this.className='txt'" style="text-align:left;" /></td>
							<td><?=$badword['admin']?></td>
						</tr>
					<? } ?>
					<tr class="nobg">
