...
<div class="menu">
	<ul id="leftmenu">
		...
		<? if($user['allowadminsetting'] || $user['isfounder']) { ?><li><a href="admin.php?m=setting&a=ls" target="main">基本设置</a></li><? } ?>
		<? if($user['allowadminsetting'] || $user['isfounder']) { ?><li><a href="admin.php?m=setting&a=register" target="main">注册设置</a></li><? } ?>
		<? if($user['allowadminapp'] || $user['isfounder']) { ?><li><a href="admin.php?m=app&a=ls" target="main">应用管理</a></li><? } ?>
		<? if($user['allowadminuser'] || $user['isfounder']) { ?><li><a href="admin.php?m=user&a=ls" target="main">用户管理</a></li><? } ?>
		<? if($user['isfounder']) { ?><li><a href="admin.php?m=admin&a=ls" target="main">管理员</a></li><? } ?>
		<? if($user['allowadminpm'] || $user['isfounder']) { ?><li><a href="admin.php?m=pm&a=ls" target="main">短消息</a></li><? } ?>
		<? if($user['allowadmincredits'] || $user['isfounder']) { ?><li><a href="admin.php?m=credit&a=ls" target="main">积分兑换</a></li><? } ?>
		<? if($user['allowadminbadword'] || $user['isfounder']) { ?><li><a href="admin.php?m=badword&a=ls" target="main">词语过滤</a></li><? } ?>
		<? if($user['allowadmindomain'] || $user['isfounder']) { ?><li><a href="admin.php?m=domain&a=ls" target="main">域名解析</a></li><? } ?>
		<? if($user['allowadmindb'] || $user['isfounder']) { ?><li><a href="admin.php?m=db&a=ls" target="main">数据备份</a></li><? } ?>
		<? if($user['allowadminnote'] || $user['isfounder']) { ?><li><a href="admin.php?m=note&a=ls" target="main">通知管理</a></li><? } ?>
		<? if($user['allowadminlog'] || $user['isfounder']) { ?><li><a href="admin.php?m=log&a=ls" target="main">日志管理</a></li><? } ?>
		<? if($user['allowadmincache'] || $user['isfounder']) { ?><li><a href="admin.php?m=cache&a=update" target="main">更新缓存</a></li><? } ?>
	</ul>
</div>
<div class="footer">Powered by UCenter <?=UC_VERSION?><br />&copy; 2001 - 2008 <a href="http://www.comsenz.com/" target="_blank">Comsenz</a> Inc.</div>
