...
<div class="container">
	<? if($a == 'ls') { ?>
		...
		<? if(!$status) { ?>
			<div class="note fixwidthdec">
				<p class="i">如果出现 <img src="images/error.gif" /> <font color="red">通信失败</font>，请点击“编辑”尝试设置应用域名对应的 IP。</p>
			</div>
		<? } elseif($status == '2') { ?>
					...
					<table class="datalist fixwidth" onmouseover="addMouseEvent(this);">
						...
						<? foreach((array)$applist as $app) {?>
							<tr>
								...
								<td width="90"><div id="status<?=$app['appid']?>">正在连接...</div><script defer="true" src="admin.php?m=app&a=ping&inajax=1&url=<? echo urlencode($app['url']);?>&ip=<? echo urlencode($app['ip']);?>&appid=<?=$app['appid']?>" frameborder="0" width="50" height="30"></script></td>
					...
					<table class="opt">
						...
						<tr>
							<td>
								...
								<input type="radio" class="radio" id="no" name="synlogin" value="0" /><label for="no">否</label>
							</td>
						...
						<tr>
							<td>
								...
								<input type="radio" class="radio" id="no" name="recvnote" value="0" /><label for="no">否</label>
							</td>
							<td></td>
						</tr>
					</table>
	...
	<? } else { ?>
		...
		<? } else { ?>
			<div class="note"><p class="i">如果出现 <img src="images/error.gif" /> <font color="red">通信失败</font>，请点击“编辑”尝试设置应用域名对应的 IP。</p></div>
		<? } ?>
		<div class="mainbox">
