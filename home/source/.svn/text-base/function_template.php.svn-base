...
/*
	...
	$Id: function_template.php 6565 2008-03-14 09:26:09Z liguode $
*/
...
function parse_template($tpl) {
	...
	//换行
	$template = preg_replace("/ \?\>[\n\r]*\<\? /s", " ", $template);
	$template = "<?php if(!defined('IN_UCHOME')) exit('Access Denied'); ?>$template";
	...
	//包含模板文件更新检查
	$template .= "<?php subtplcheck('".implode('|', $_SGLOBAL['sub_tpls'])."', '$_SGLOBAL[timestamp]', '$tpl'); ?>";
