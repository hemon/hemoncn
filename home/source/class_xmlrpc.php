...
/*
	...
	$Id: class_xmlrpc.php 7591 2008-06-13 10:29:21Z liguode $
*/
...
class xmlrpc {
	...
	function authUser($username, $password) {
		...
		$username = siconv($username, $this->charset, 'UTF-8');
		$password = siconv($password, $this->charset, 'UTF-8');
	...
	function getClassId($uid, $classname) {
		$class = array();
		if($classname) {
			$classname = siconv($classname, $this->charset, 'UTF-8');
	...
	function opPost($username, $password, $post, $postid=0) {
		...
		$blog_post_data = array(
			'classid' => $this->getClassId($uid, $post['categories'][0]),
			'subject' => $post['title'],
			'message' => $post['description'],
			'tag' => empty($post['tagwords'])?'':siconv(implode(' ', $post['tagwords']), $this->charset, 'UTF-8')
		);
