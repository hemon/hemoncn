...
/*
	...
	$Id: admincp_block.php 7157 2008-04-23 02:47:25Z liguode $
*/
...
if(submitcheck('valuesubmit')) {
	...
	if($setarr['blocksql'] && !$_SGLOBAL['db']->query($setarr['blocksql'].' LIMIT 1', 'SILENT')) {
		cpmessage('sql_statements_can_not_be_completed_for_normal');
	}
	...
	//下一步
	showmessage('enter_the_next_step', $turl.'&op=code&id='.$bid, 0);
	...
	if($block['blocksql']) {
		if(empty($setarr['perpage'])) {
			...
			$block['blocksql'] .= " LIMIT 0,$setarr[num]";
		} else {
...
if(empty($_GET['op'])) {
	...
	if(!empty($block['blocksql'])) {
		if($query = $_SGLOBAL['db']->query($block['blocksql']." LIMIT 1", 'SILENT')) {
