...
/*
	...
	$Id: admincp_backup.php 7399 2008-05-19 04:44:32Z zhengqingpeng $
*/
...
//权限
if(!checkperm('managebackup')) {
	cpmessage('no_authority_management_operation');
}
//获取分卷编号
$volume = isset($_GET['volume']) ? (intval($_GET['volume']) + 1) : 1;
$excepttables = array($_SC['tablepre'].'session');
...
if(empty($_GET['op'])) {
	...
	if(empty($_GET['filename']) || strlen($_GET['filename']) > 40){	//文件名长度判断
		cpmessage('documents_were_incorrect_length');
	} else {
		$_GET['filename'] = preg_replace("/[^a-z0-9_]/i", '',(str_replace('.', '_', $_GET['filename'])));
	}

	$tables = array();
	//备份方式
	if($_GET['type'] == 'uchomes') {
		$tables = arraykeys2(fetchtablelist($tablepre), 'Name');
	} elseif($_GET['type'] == 'custom') {
		if(isset($_GET['setup'])) {//POST提交备份
			$tables = empty($_GET['customtables']) ? array() : $_GET['customtables'];
			$tablesnew = addslashes(serialize($tables));
			data_set('custombackup', $tablesnew);
		} else {
			//自动跳转备份
			data_get('custombackup');
	...
	$idstring = '# Identify: '.base64_encode("$_SGLOBAL[timestamp],".X_VER.",$_GET[type],$_GET[method],$volume")."\n";
	$dumpcharset = $_GET['sqlcharset'] ? $_GET['sqlcharset'] : str_replace('-', '', $_SC['charset']);
	$setnames = ($_GET['sqlcharset'] && $_SGLOBAL['db']->version() > '4.1' && (!$_GET['sqlcompat'] || $_GET['sqlcompat'] == 'MYSQL41')) ? "SET NAMES '$dumpcharset';\n\n" : '';

	if($_SGLOBAL['db']->version() > '4.1') {
		if($_GET['sqlcharset']) {
			$_SGLOBAL['db']->query("SET NAMES '".$_GET['sqlcharset']."'");
		}
	...
	$backupfile = S_ROOT.'./data/'.$backupdir.'/'.$_GET['filename'];
	if($_GET['usezip']) {
		include_once S_ROOT.'./source/class_zib.php';
	}

	if($_GET['method'] == 'multivol') {//分卷备份
		$sqldump = '';
		$tableid = isset($_GET['tableid']) ? intval($_GET['tableid']) : 0 ;//表ID
		$startfrom = isset($_GET['startfrom']) ? intval($_GET['startfrom']) : 0 ;//起始位置
		$tablenum = count($tables);
		$filesize = $_GET['sizelimit'] * 1000;
		$complate = true;
		...
		if(trim($sqldump)) {
			...
			if(!fwrite($fp, $sqldump)) {
				...
				cpmessage('vol_backup_database', 'admincp.php?ac=backup&op=backup&type='.rawurldecode($_GET['type']).'&filename='.rawurlencode($_GET['filename']).'&method=multivol&sizelimit='.intval($_GET['sizelimit']).'&tableid='.intval($tableid).'&startfrom='.intval($startrows).'&extendins='.intval($_GET['extendins']).'&sqlcharset='.rawurlencode($_GET['sqlcharset']).'&sqlcompat='.rawurlencode($_GET['sqlcompat']).'&usehex='.intval($_GET['usehex']).'&usezip='.intval($_GET['usezip']).'&volume='.intval($volume), 1, array($volume));//debug
			}
		} else {
			if($_GET['usezip'] == 1){
...
function fetchtablelist($tablepre = '') {
	global $_SGLOBAL, $excepttables, $_SC;
	!$tablepre && $tablepre = '*';
	$tables = $table = array();
...
function sqldumptable($table, $startfrom = 0, $currsize = 0) {
	global $_SGLOBAL, $filesize, $startrows, $_GET, $dumpcharset, $complate, $excepttables;
